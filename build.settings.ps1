...
Properties {
    ...
    $ModuleRootDir  = "$PSScriptRoot\$ModuleName"
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    ...
    $InstallPath = Join-Path (Split-Path $(if ($profile) {$profile} else {$Home}) -Parent) `
                             "Modules\$ModuleName\$((Test-ModuleManifest -Path $ModuleRootDir\$ModuleName.psd1).Version.ToString())"
    ...
    # the module. However, if there are such files, add their $ModuleRootDir relative paths to the exclude list.
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $Exclude = @()
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage('PSUseDeclaredVarsMoreThanAssigments', '')]
    $CodeCoverageFiles = "$ModuleRootDir\*.ps1", "$ModuleRootDir\*.psm1"
